...
final class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        navController = UINavigationController(rootViewController: makeSplashVC())
        window?.rootViewController = navController
        window?.makeKeyAndVisible()
    }

    private func makeSplashVC() -> SplashViewController {
        let viewModel = SplashViewModel()

        viewModel.finishedLoadingBeers.sink { [weak self] beers in
            self?.showHomeViewController(with: beers)
